    ...
    return sales.filter(tx => {
      ...
      if (statusFilter !== 'All') {
        if (statusFilter === 'COMPLETED') {
          matchStatus = !tx.status || tx.status === 'COMPLETED';
        } else {
      ...
      return matchSearch && matchPayment && matchTerminal && matchStatus && matchDate;
    });
  }, [sales, search, paymentFilter, terminalFilter, statusFilter, dateFilter]);
                        ...
                        <td className="py-3 px-4 text-right whitespace-nowrap">
                          <div className="flex items-center justify-end space-x-1.5">
                            ...
                            {canRefund && (
                              tx.status === 'REFUNDED' ? (
                                <span
                                  className="px-2.5 py-1 bg-slate-100 text-slate-400 rounded text-[11px] font-semibold flex items-center space-x-1 cursor-not-allowed border border-slate-200"
                                  title="This sale transaction has already been fully refunded"
                                >
                                ...
                                <button
                                  ...
                                  className={`px-2.5 py-1 rounded text-[11px] font-semibold flex items-center space-x-1 cursor-pointer transition-colors ${
                                    tx.status === 'PARTIALLY_REFUNDED'
                                      ? 'bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200'
                                      : 'bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200'
                                  }`}
                                  title={tx.status === 'PARTIALLY_REFUNDED' ? 'Process additional item return' : 'Process customer return / refund'}
                                >
                                  <RotateCcw className="w-3.5 h-3.5" />
                                  <span>{tx.status === 'PARTIALLY_REFUNDED' ? 'Refund More' : 'Refund'}</span>
                                </button>
                              )
                            )}
                          </div>
